 ...
 * @since 4.5.0
 */
class ExportMediaLibrary extends base\Base {
    ...
    public function rest_api_init() {
        register_rest_route(rest\Service::SERVICE_NAMESPACE, '/massedge/export', array(
            'methods' => 'GET',
     ...
     * @apiParam {('wosFlat'|'wosHierarchical'|'wsFlat'|'wsHierarchicalRML'|'wsHierarchical')} type The export type
    ...
    public function export($request) {
        ...
        if (($permit = rest\Service::permit()) !== null) return $permit;
        $type = $request->get_param('type');
        $folder = wp_rml_get_object_by_id($request->get_param('folder'));
        
        if (!is_rml_folder($folder)) {
            return new \WP_Error('rest_rml_massedge_export', __('No valid folder.'), array('status' => 500));
        }
        ...
        switch($type) {
            ...
            case 'wsHierarchical':
                ...
                break;
            default:
                return new \WP_Error('rest_rml_massedge_export', __('No valid type.'), array('status' => 500));
        }
