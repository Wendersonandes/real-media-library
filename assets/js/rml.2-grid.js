...
window.rml.hooks.register("tree/switch", function(obj, id, type, $) {
    ...
    backbone && backbone.view.$el.parents('.media-modal').size() === 0 && backbone.view.controller.state().get('selection').reset();
});
...
/**
 * Lazy load the folders in select option for attachments in grid view.
 * This is ported from v4 and needed in v3 hotfix.
 * 
 * @deprecated in v4, hotfix for v3.4.7
 */
window.rml.hooks.register('_attachment_fields_to_edit', function(selector, selectedId, $) {
    var $select = $('select[name="' + selector + '"]');
    
    // Add empty options for lazy loading of folders
    var optionsPlaceholder = (function(p, cnt) {
        var result = "";
        for (var i = 0; i < cnt; i++) {
            result += p;
        }
    ...
    $select.append(optionsPlaceholder);
    
	$select.one('click', function() {
	    $.post(rmlOpts.ajaxUrl, {
	...
	if ($select.parents("#post-body").length) {
